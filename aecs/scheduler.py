...
class SignalBuffer:
    ...
    def push_grad_cosine(self, grad_flat: torch.Tensor):
        if self._prev_grad_flat is not None:
            # Performance optimization: defer .item() extraction to prevent
            # CPU-GPU sync. Using .detach() ensures we don't store computation
            # graphs in memory.
    ...
    def redundancy_score(self) -> float:
        ...
        if isinstance(self.grad_cosines[0], torch.Tensor):
            # Performance optimization: stack tensors for single-kernel reduction,
            # handling mixed types (floats injected by tests) gracefully.
            device = self.grad_cosines[0].device
            tensors = [
