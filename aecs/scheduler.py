...
class SignalBuffer:
    ...
    def push_grad_cosine(self, grad_flat: torch.Tensor):
        if self._prev_grad_flat is not None:
            # Performance optimization: Defer .item() extraction to avoid
            # CPU-GPU sync blocks. Use .detach() to prevent storing graphs.
    ...
    def redundancy_score(self) -> float:
        ...
        # Check if any element is a PyTorch tensor
        if any(isinstance(x, torch.Tensor) for x in self.grad_cosines):
            # Performance optimization: single-kernel reduction for tensors
            # Gracefully handle mixed types (floats in tests, tensors in prod)
            device = next(
